        ...
        <div class="px-6 py-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative w-12 h-12 rounded-lg bg-bg-surface-alt border border-border-subtle flex items-center justify-center">
                    ...
                    <div>
                        <div class="flex items-center gap-3">
                            ...
                            <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium {{ $server->is_active ? 'bg-primary-500/10 text-primary-600' : 'bg-bg-surface-alt text-text-tertiary' }}">
                ...
                @foreach ($tabs as $key => $label)
                    <button
                        ...
                        @class([
                            ...
                            'border-transparent text-text-secondary hover:text-text-primary hover:border-border-strong' => $activeTab !== $key,
                        ])>
                        {{ $label }}
                    </button>
                @endforeach
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            ...
                            <div class="bg-bg-surface border border-border-subtle rounded-xl p-5">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <svg class="w-4 h-4 text-text-tertiary" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                </div>
                                <div class="text-3xl font-bold text-text-primary mb-2">{{ number_format($server->cpu_usage ?? 0, 1) }}%</div>
                                <div class="h-1.5 w-full bg-bg-surface-alt rounded-full overflow-hidden">
                                    <div class="h-full bg-primary-500 rounded-full transition-all duration-500" style="width: {{ min($server->cpu_usage ?? 0, 100) }}%"></div>
                                </div>
                            </div>
                            ...
                            <div class="bg-bg-surface border border-border-subtle rounded-xl p-5">
                                ...
                                <div class="text-3xl font-bold text-text-primary mb-2">{{ number_format($memUsed / 1024, 1) }} <span class="text-sm font-normal text-text-tertiary">GB</span></div>
                                <div class="h-1.5 w-full bg-bg-surface-alt rounded-full overflow-hidden">
                                    <div class="h-full bg-orange-500 rounded-full transition-all duration-500" style="width: {{ min($server->memory_usage ?? 0, 100) }}%"></div>
                                </div>
                            </div>
                            ...
                            <div class="bg-bg-surface border border-border-subtle rounded-xl p-5">
                                ...
                                <div class="h-1.5 w-full bg-bg-surface-alt rounded-full overflow-hidden">
                                    <div class="h-full bg-amber-500 rounded-full transition-all duration-500" style="width: {{ min($server->disk_usage ?? 0, 100) }}%"></div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-bg-surface border border-border-subtle rounded-xl p-5">
                            ...
                            <dl class="space-y-3 text-sm">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <dd class="text-text-primary font-medium">{{ $server->kernel_version ?? 'Unknown' }}</dd>
                                </div>
                                <div class="flex justify-between">
                                    <dt class="text-text-tertiary">Last Updated</dt>
                                    <dd class="text-text-primary font-medium">{{ $server->last_detail_fetch_at ? $server->last_detail_fetch_at->diffForHumans() : 'Never' }}</dd>
                                </div>
                            </dl>
                        </div>
                        ...
                        <div class="bg-bg-surface border border-border-subtle rounded-xl p-5">
                            ...
                            <div class="space-y-2">
                                <button class="w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium text-text-secondary hover:bg-bg-surface-alt hover:text-text-primary transition-colors">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0l3.181 3.183a8.25 8.25 0 0013.803-3.7M4.031 9.865a8.25 8.25 0 0113.803-3.7l3.181 3.182m0-4.991v4.99" />
                                    </svg>
                                    Restart Server
                                </button>
                                <button class="w-full flex items-center gap-2 px-3 py-2 rounded-md text-sm font-medium text-danger hover:bg-red-900/20 transition-colors">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                    ...
                    <div class="divide-y divide-border-subtle">
                        @forelse ($this->commandLogs as $log)
                            <div class="px-5 py-3 flex items-start gap-3 hover:bg-bg-surface-alt transition-colors">
                                <div class="mt-1 text-text-tertiary font-mono text-xs">{{ $log->created_at->format('H:i:s') }}</div>
                        ...
                        @forelse ($this->activityLogEntries as $activity)
                            <div class="px-5 py-4 flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-text-tertiary mt-0.5">{{ $activity->created_at->diffForHumans() }}</p>
                                </div>
                            </div>
                        @empty
